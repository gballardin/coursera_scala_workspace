...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    StructType(Array(StructField(columnNames(0), StringType, false)) ++ columnNames.drop(1).toArray.map(fieldName => StructField(fieldName, DoubleType, false)))
  }

  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    * @param line Raw fields
    */
  def row(line: List[String]): Row = Row.fromSeq(line(0).trim :: line.drop(1).map(x => x.trim.toDouble))
